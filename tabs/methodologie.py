...
def methodo():
    ...
    texte = """
    - Contrairement à la partie Datavisualisation, nous avons garder les codes de chaques pays pour nous éviter de les encoder par la suite. Donc chaque pays à un code attribué.
    - Nous n'avons garder que la modalité "meteorogical year" qui permet d'avoir une valeur par année pour chaque pays.
    """
    st.markdown(texte)
    ...
    st.write("Visiblement, les résultats de cette régréssion ne sont pas très bons avec unen RMSE à 0.58 mais surtout un coéfficient de determination **négatif**. Cela montre que le modèle prend mal en compte la variance de la série.")
    ...
    st.write("**Insérer le prophet de JM.**")
    st.write("**Conclure pour garder que le plus performant.**")
    ...
    plt.title('Régression Linéaire pour la prédiction des emissions de co2')
    st.pyplot(plt)
    st.write("Nous voyons ici que lorsque les valeurs d'emissions sont faibles, le modèle est plutôt efficace en suivant la ligne de régréssion. En revanche, plus les emissions sont importantes, plus le modèle s'affaiblit")
    texte = """
    - Maintenant, évaluons notre modèle pour pouvoir le comparer avec les autres:
    """
    st.markdown(texte)
    ...
    st.markdown("<u>*C. Le Random Forest et le l'abre de décision:*</u>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.pyplot(fig)
    st.write("Les deux modèles semblent globalement identique. Ce que l'on peut dire, c'est que les points sont plus proches de la ligne de régression et surtout pour les plus grandes emissions. Cela signifie que les modèles sont plus performants que la régréssion linéaire")
    ...
    st.write("**Garder les deux modèles et passer à la démonstration des prédictions avec le modèle le plus performant.**")
    st.write("Une fois que l'on aura identifié les deux modèles les plus performants, s'en servire pour faire des prédictions jusqu'en 2050. Les concaténer dans un dataframe")
